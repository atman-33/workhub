...
pub struct Settings {
    ...
    /// `_ai/comms/`. Consulting costs tokens, so it is **off by default**
    /// (T-0158) and turned on deliberately. Read by the plugin hooks from this
    /// config file; the app only stores the flag.
    #[serde(default)]
