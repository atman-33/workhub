...
fn main() {
    ...
    if env::var("WHISPER_DONT_GENERATE_BINDINGS").is_ok() {
        ...
        // On windows-gnu, libclang knows nothing about the mingw toolchain's
        // header locations; feed it the include search path reported by gcc so
        // binding generation works without BINDGEN_EXTRA_CLANG_ARGS.
        if target.contains("windows-gnu") {
            bindings = bindings.clang_arg(format!("--target={}", target));
            for dir in mingw_include_dirs() {
    ...
    if cfg!(target_os = "windows") {
        // /utf-8 is an MSVC-only flag; passing it to mingw g++ breaks the
        // cmake compiler test on windows-gnu (g++ parses it as a file name).
    ...
    // whisper.cpp's ggml/CMakeLists.txt strips the `lib` prefix from static
    // archives on WIN32 even for mingw, producing `ggml.a` etc., which the GNU
    // linker (and rustc's library preflight) cannot resolve as `-lggml`.
    // Mirror each bare `<name>.a` as `lib<name>.a` so linking works.
    if target.contains("windows-gnu") {
        add_lib_prefix_copies(&out.join("build")).unwrap();
        add_lib_prefix_copies(&destination).unwrap();
    }
...
/// Ask the mingw gcc on PATH (or $CC) for its default `#include <...>` search
/// directories. Returns an empty list if gcc is unavailable or the output
/// cannot be parsed — bindgen then fails and the bundled bindings are used.
fn mingw_include_dirs() -> Vec<String> {
...
/// Recursively copy every static archive named `<name>.a` (no `lib` prefix)
/// to `lib<name>.a` alongside it, skipping copies that already exist. See the
/// windows-gnu note at the call site.
fn add_lib_prefix_copies(dir: &std::path::Path) -> std::io::Result<()> {
    if !dir.is_dir() {
        return Ok(());
    }
    for entry in std::fs::read_dir(dir)? {
        let path = entry?.path();
        if path.is_dir() {
            add_lib_prefix_copies(&path)?;
        } else if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
            if name.ends_with(".a") && !name.starts_with("lib") {
                let target = path.with_file_name(format!("lib{}", name));
                if !target.exists() {
