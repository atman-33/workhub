...
に答えるときは、訓練データや今の文脈からではなく先に記憶を検索する。蒸留済みの
\`memory/\` が先、会話の逐語が後（\`memory-recall\` が両方を引く）。答えに形が
あるなら型で絞る（未決の判断なら \`notes --type decision --status open\`、
進行中なら \`notes --type session --status open\`）。形が無い開かれた問いは語で探し、
それでも無ければ会話の逐語を意味検索する。
