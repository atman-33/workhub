...
This is an audit trail, not a policy. When a line here turns out to be wrong,
the fix is to correct `profile/decision-policy.md` — that is what agents judge
from. When a line keeps recurring, it has become a standing rule and belongs in
the policy's `## Past decisions` or `## Preferences`.
