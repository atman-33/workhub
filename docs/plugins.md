...
|---|---|---|---|
| `workhub` | **Required** | user or project | Task-board skills (`task-list`, `task-start`, `task-report`, `vault-init`, `vault-setup`), vault layout migrations (`vault-upgrade` — carries an existing vault through a structural change, one migration at a time: it shows what would move, refuses a dirty worktree, never deletes and never overwrites, and is a no-op once done; a vault left behind by such a change breaks silently, since the hooks gate on paths that no longer exist), project onboarding (`project-start` — the project-level counterpart of `task-start`: reads `projects/NNNN-<slug>/README.md` first, follows its reading order only as far as the request needs, resolves the project's `repos:`, and reports goal / status / source / work in flight; read-only), team shared spaces (`shared-space` — surveys a knowledge base that lives outside the vault, a network drive or a Drive/SharePoint folder, and records how the team organises it in `projects/NNNN-<slug>/shared/`; the Projects tab reads those notes and offers the prompt that launches it), schedule editing (`schedule-edit` — rewrites `projects/NNNN-<slug>/schedules/*.md` from a natural-language instruction; launched by the app's Schedule tab), mindmap editing (`mindmap-edit` — rewrites `projects/NNNN-<slug>/mindmaps/*.md` from a natural-language instruction; launched by the app's Mindmap tab), vault knowledge-base skills (`kb-ingest`, `kb-query`, `kb-lint`, `kb-index` — they own the vault's inbox/projects/knowledge/archive layout), the `strategist` skill (a counsel that reads `strategy/` — the owner's north star, present state and bottlenecks — argues with the gaps between them, and writes the conclusions back; it borrows a character from `persona` when that plugin is installed and runs unstyled when it is not), the workhub app's own release procedure (`release-app` — bump, changelog, tag, push, verify the published assets), vault write-guard / task-sync hooks, and the harness hooks in `hooks/harness/` (`inject-project-context` — the sole reader of the `.claude/project-context.json` the app writes; `inject-target-rules` and `inject-extended-rules` — the sibling-repository CLAUDE.md and `rules-ex` injection Claude Code does not do on its own), together with the three skills that write what those hooks read: `setup-project-context`, `setup-rules-ex` and `capture-rule`. Hooks and skills came from `engineering`, which stopped being required the moment they left it — a setup step for a required plugin cannot sit inside an optional one, which is exactly what made `rules-ex` unusable for a team that switches the development workflow off. Meaningless outside a vault, and every hook here gates on finding one. |
| `engineering` | Recommended | **user** | Development workflow: role-based sub-agents (`code-explore`, `implementer`, `heavy-implementer`, `test-runner`), the `inject-role-delegation` hook that injects the criteria for using them, `post-format-project`, and skills (commit, PR, branch, ADR, system design documents, bug investigation, review/test/onboarding guides, …). Test-first development, codebase design, spec/ticket flows and plan grilling are not here: they used to ship as vendored copies of `mattpocock/skills` v1.0.1 and were removed in favor of the upstream `mattpocock-skills@claude-plugins-official` plugin, which carries the current versions (`tdd`, `codebase-design`, `to-spec`, `to-tickets`, `grill-with-docs`, …) — install it at user scope alongside this one to keep those workflows. The serena and context7 MCP launchers it used to carry left for `mcp-serena` and `mcp-context7` (below) — an MCP server can only be switched off by switching off the plugin that registers it, so bundling them here meant paying for a Python toolchain and a pair of language servers to get a commit-message skill, and a serena that failed to start looked like an engineering-workflow failure. The sub-agents still prefer serena's symbol-aware tools when it is enabled and fall back to `Grep`/`Glob`/`Edit` when it is not; nothing here requires either server. Recommended rather than required because commit messages, branch names and review flow are a team's own conventions, and a team on different ones should be able to switch this off without breaking the app — which is why the app-coupled rule-injection hooks, and the `setup-project-context` / `setup-rules-ex` / `capture-rule` skills that feed them, now live in `workhub`. `setup-all` stays here and delegates to the first two, skipping them with a note when `workhub` is absent. The delegation hook stayed here on purpose: it reads the same `project-context.json`, but the criteria it injects name the sub-agents above, so disabling this plugin has to take them with it. Releasing is deliberately not here — a release procedure is repository-specific, so it belongs in that repository's `.claude/rules/` plus a repository-specific skill (e.g. `workhub`'s `release-app`). |
| `mcp-serena` | Optional | **user** | The [serena](https://github.com/oraios/serena) MCP server and nothing else: symbol-aware code retrieval and editing (`find_symbol`, `find_referencing_symbols`, `replace_symbol_body`, …), started through a Node launcher that branches on platform so one `.mcp.json` serves Windows and WSL. Needs `uv`/`uvx` on `PATH`, pulls Serena from git on first launch, and starts a language server per language — by far the heaviest thing in this marketplace, which is the whole reason it is separately switchable. Its README carries the troubleshooting for the failure mode that is easiest to misread: an old `~/.serena/serena_config.yml` carrying `token_count_estimator: TIKTOKEN_GPT4O` makes the agent download a tiktoken encoding at startup, which dies with an SSL error on any machine whose TLS is intercepted, and takes the server down with it (T-0283). |
| `mcp-context7` | Optional | **user** | The [context7](https://github.com/upstash/context7) MCP server and nothing else: current library and framework documentation, so an answer about a dependency comes from its docs rather than from training data. Needs only Node — it falls back to `npx -y @upstash/context7-mcp`, and a global install just makes startup quicker. No API key. Its own plugin rather than a shared `mcp` one with `mcp-serena`: the two have nothing in common but the transport, and bundling them would mean switching serena off takes context7 with it — which recreates, one level down, exactly the problem the split was made to solve. |
| `claude-tooling` | Optional | **user** | Extending and maintaining Claude Code itself: `create-claude-command`, `writing-great-skills`, `install-skill`, `manage-desktop-routines`, and `grilling` (stress-testing a plan — the tool you reach for while shaping what a command or skill should do). Carries the `SessionStart` plugin-update notice, which reports outdated plugins across every marketplace. That notice is the whole story of this plugin's tier: it was why the plugin used to be required, then the app's **Plugins** tab answered the same question — for every registered marketplace since T-0238, not just this one — in one place and without a session, which left a recommendation resting on nothing. What remains is authoring commands and skills and stress-testing a plan: useful, and a matter of whether you extend Claude Code at all, which is what optional means here (T-0297). No vault or project-context dependency. |
...
| `writing` | Optional | user | Natural Japanese business writing: `natural-japanese` (style constitution, doctype guides, manual review checklist — a selective, script-free port of coji/natural-japanese, see `plugins/writing/NOTICE.md`). The home for future writing-quality skills; platform-specific writing stays in `zenn`. No vault or project-context dependency. |
| `thinking` | Optional | user | Skills that ask instead of answer, so the understanding or the structure ends up in the owner's head: `teach-back` (quizzes you on a document, one question at a time, until you can explain it to someone else — definitions, numbers, evidence, background, scope — and logs the document's own gaps apart from yours; a rewrite borrowing ideas from four MIT skills, see `plugins/thinking/NOTICE.md`) and `ladder` (structures a concept `down` into definition, purpose, desired state, components and observable conditions, or climbs `up` from a symptom through facts and causes to the purpose and the state worth aiming for, closing each level on the owner's explicit agreement). Kept apart from `grilling`, which questions a plan's decisions and offers the answer, and from `strategy-decompose`, which starts from a strategy handed down and audits it. No scripts, no vault or project-context dependency. |
| `strategy` | Optional | project or user | Turning a strategy handed down from above into your own organization's target states: `strategy-decompose` (take in the upper strategy, close the unknowns with the owner, decompose it into aspects and states, audit, output an xlsx) plus the `strategy-auditor` agent, which checks a decomposition for necessity, sufficiency and overlap from a context other than the one that wrote it. Ships `build_xlsx.mjs` / `read_xlsx.mjs` so the workbook shape does not drift between runs. No vault or project-context dependency. |
...
| `memory` | Recommended | project or user | Long-term memory for agent sessions on a workhub vault (`memory-setup`, `memory-recall`, `memory-doctor` + capture/inject hooks backed by the bundled `engine/`). Every session's Q&A pairs are captured into `<vault>/_ai/state/memory.db` and the relevant ones are injected into later sessions, found by hybrid keyword + vector search. Fully local: a Japanese-capable embedding model (Ruri v3) runs on the CPU, nothing leaves the machine and no LLM is called. Split out of `workhub` in T-0367 so memory can be switched off on its own — it is the one part of the harness with a per-machine setup step (~320 MB model) and a database of its own, and a vault that does not want either should not have to give up the task board to say so. Needs `memory-setup` once per machine; until then the hooks stay silently disabled and the app shows a reminder. |
...
claude plugin install mcp-serena@workhub-marketplace
claude plugin install mcp-context7@workhub-marketplace
...
### Migrating from `engineering` 0.26 and earlier

`engineering` used to register the `serena` and `context7` MCP servers itself.
From 0.27 it registers none, so a machine that had them through that plugin
loses both on the next update and has to enable the two new plugins to get them
back:
...
Nothing else changes: the launchers moved unmodified, the server names are the
same, and `engineering`'s sub-agents work with or without them.
