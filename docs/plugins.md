...
- **required**: an app feature or an agent launch stops working. Only two
  plugins qualify. `workhub` is named directly in the app's own launch prompts
  (`task-start`, `task-report`, `/kb-ingest`) and owns the skills the Schedule
  and Mindmap tabs run; `engineering`'s SessionStart hook is the sole reader of
  the `.claude/project-context.json` the app writes, so without it the
  repositories registered in the app never reach an agent at all.
- **recommended**: nothing breaks, but the harness is noticeably worse. A tab
  that explains its own absence (`persona`) belongs here, not above it.
...
| `obsidian` | Optional | project or user | Generic Obsidian format helpers (Obsidian Flavored Markdown, Bases, JSON Canvas, Obsidian CLI, defuddle). Pre-enabled by the vault template. Vault-agnostic — useful in the workhub vault and any other vault. |
...
claude plugin install persona@workhub-marketplace         # user scope
